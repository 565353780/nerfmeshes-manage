...
def extract_radiance(model, args, device, nums):
    ...
    if isinstance(nums, int):
        ...
        assert (len(nums) == 3
                ), "Nums arg should be of length 3, number of axes for 3D"
    ...
    radiance_samples = []
    for (samples, ) in batchify(samples,
    ...
    radiance = torch.cat(radiance_samples, 0).view(*nums,
                                                   4).contiguous().numpy()
...
def extract_iso_level(density, args):
    ...
    print(
        f"Min density {min_a}, Max density: {max_a}, Mean density {density.mean()}"
    )
...
def extract_geometry(model, device, args):
    ...
    vertices, triangles, normals, _ = [
        torch.from_numpy(np.ascontiguousarray(result)) for result in results
    ]
...
def export_marching_cubes(model, args, cfg, device):
    ...
    if args.use_cached_mesh and cached_mesh_exists:
        ...
        vertices, triangles, normals, density = extract_geometry(
            model, device, args)

        if cache_new_mesh or args.override_cache_mesh:
            torch.save((vertices, triangles, normals, density),
                       mesh_cache_path)
    ...
    if args.no_view_dependence:
        ...
        batch_generator = batchify(targets,
                                   directions,
                                   batch_size=args.batch_size,
                                   device=device)
        ...
        ray_bounds = torch.tensor([0., args.view_disparity_max_bound],
...
def demo():
    ...
    parser.add_argument(
        ...
        default=None,
        help=
        "Training log path with the config and checkpoints to load existent configuration.",
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
        ...
        help=
        "Load existent configuration from the latest checkpoint by default.",
    )
    parser.add_argument(
        "--save-dir",
        type=str,
    ...
    parser.add_argument(
        ...
        default=128,
        help=
        "Sampling resolution for marching cubes, increase it for higher level of detail.",
    )
    parser.add_argument(
        "--super-sampling",
        type=int,
    ...
    parser.add_argument(
        ...
        default=1024,
        help=
        "Higher batch size results in faster processing but needs more device memory.",
    )
    parser.add_argument(
        "--no-view-dependence",
        action="store_true",
        default=False,
        help=
        "Disable view dependent appearance, use sampled diffuse color based on the grid"
    )
    parser.add_argument(
        "--view-disparity",
        type=float,
        default=1e-2,
        help=
        "Ray origins offset from target based on the inverse normal for the view dependent appearance.",
    )
    parser.add_argument(
        "--view-disparity-max-bound",
        type=float,
        default=4e0,
        help=
        "Far max possible bound, usually set to (cfg.far - cfg.near), lower it for better "
    ...
    parser.add_argument(
        ...
        default=False,
        help=
        "Caches the mesh, useful for rapid configuration appearance tweaking.",
    )
    parser.add_argument(
        "--cache-name",
        type=str,
        default="mesh_cache.pt",
        help=
        "Mesh cache name, allows for multiple unique meshes of different resolutions.",
    )
    config_args = parser.parse_args()

    config_args.log_checkpoint = "./logs/nerf-ustc_niu/ustc_niu/version_5/"
    config_args.checkpoint = "model_last.ckpt"
    config_args.save_dir = "./output/meshes"
    config_args.mesh_name = "mesh.obj"
    config_args.iso_level = 32
    config_args.limit = 1.2
    config_args.res = 480
    config_args.super_sampling = 0
    config_args.batch_size = 1024
    config_args.no_view_dependence = False
    config_args.view_disparity = 1e-2
    config_args.view_disparity_max_bound = 1e0
    config_args.use_cached_mesh = False
    config_args.override_cache_mesh = False
    config_args.cache_name = "mesh_cache.pt"

    # Existent log path
    path_parser = PathParser()
    cfg, _ = path_parser.parse(None, config_args.log_checkpoint, None,
    ...
    model = getattr(models, cfg.experiment.model).load_from_checkpoint(
        path_parser.checkpoint_path)
